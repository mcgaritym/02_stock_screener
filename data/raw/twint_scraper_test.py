...
# save to csv file
df_scraper.to_csv('df_twitter_scrape' + '_'.join(company_input) + str(datetime.now().strftime("%Y-%m-%d__%H-%M-%S")) + '.csv', index=False)
...
# save to csv file
df_sentiment.to_csv('df_twitter_sent' + '_'.join(company_input) + str(datetime.now().strftime("%Y-%m-%d__%H-%M-%S")) + '.csv', index=False)
